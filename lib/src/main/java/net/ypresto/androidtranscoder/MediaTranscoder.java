...
public class MediaTranscoder {
     ...
     * @deprecated Use {@link #transcodeVideo(FileDescriptor, String, MediaFormatStrategy, OutputSurfaceFactory, double, long, long, MediaTranscoder.Listener)} which accepts output video format.
     */
    @Deprecated
    public Future<Void> transcodeVideo(final FileDescriptor inFileDescriptor, final String outPath, final OutputSurfaceFactory outputSurfaceFactory, final double playbackSpeed, final long startMs, final long endMs, final Listener listener) {
        ...
        }, outputSurfaceFactory, playbackSpeed, startMs, endMs, listener);
    }
        ...
        final Future<Void> createdFuture = mExecutor.submit(new Callable<Void>() {
            ...
            public Void call() throws Exception {
                ...
                try {
                    ...
                    engine.setTrimStart(startMs);
                    engine.setTrimEnd(endMs);
     ...
     * @param listener          Listener instance for callback.
     */
    public Future<Void> transcodeVideoWithoutAudio(final FileDescriptor inFileDescriptor,
        ...
        final Future<Void> createdFuture = mExecutor.submit(new Callable<Void>() {
            ...
            public Void call() throws Exception {
                ...
                try {
                    ...
                    engine.setTrimStart(startMs);
                    engine.setTrimEnd(endMs);
                    engine.transcodeVideoWithoutAudio(outPath, outFormatStrategy, outputSurfaceFactory);
                } catch (IOException e) {
                    Log.w(TAG, "Transcode failed: input file (fd: " + inFileDescriptor.toString() + ") not found"
                            + " or could not open output file ('" + outPath + "') .", e);
                    caughtException = e;
